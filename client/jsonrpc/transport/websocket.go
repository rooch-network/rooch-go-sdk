...
import (
	...
	"github.com/rooch-network/rooch-go-sdk/client/jsonrpc/codec"
)
...
func (s *stream) listen() {
	...
	for {
		...
		if id != 0 {
			...
			if respSub.Method == "rooch_subscription" {
...
func (s *stream) unsubscribe(context context.Context, id string) error {
	...
	var result bool
	if err := s.Call(context, "rooch_unsubscribe", &result, id); err != nil {
		return err
	}
	if !result {
...
func (s *stream) Subscribe(context context.Context, callback func(b []byte), parms interface{}) (func() error, error) {
	...
	if err := s.Call(context, "rooch_subscribe", &out, parms); err != nil {
		return nil, err
	}
