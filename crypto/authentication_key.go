...
const (
	Ed25519DeriveScheme DeriveScheme = 0 // Ed25519Scheme is the default scheme for deriving the AuthenticationKey
...
func (ak *AuthenticationKey) FromBytesAndScheme(bytes []byte, scheme DeriveScheme) {
	authBytes := address.Sha3256Hash([][]byte{
		bytes,
		{scheme},
	})
	copy((*ak)[:], authBytes)
